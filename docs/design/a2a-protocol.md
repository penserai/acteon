...
**Status:** Complete — all six phases shipped (PRs #160–#192).
**Author:** Acteon Team
**Created:** 2026-05-14
**Updated:** 2026-05-22 (Phase 6 + hardening complete)

> **Implementation note.** Every phase below is shipped and merged.
> Phases 1–4 deliver the protocol (core types, Task Engine, Discovery
> & SSE, Push & Security Schemes); Phase 5 hardens it (adversarial
> tests, `proptest` codec fuzzing, a `criterion` load benchmark, and
> a security review whose one finding — SSRF in the push delivery
> worker — was fixed); Phase 6 ships the five polyglot SDKs, the
> `a2a_core_simulation` example, and the user-facing docs. Two
> review-driven extras also landed: a 60s discovery cache and a
> persistent push-delivery DLQ. The only checklist item not built is
> per-tenant cap overrides, assessed as not needed for v1 (see
> Phase 5).
...
- [x] **The Bridge:** `crates/gateway/src/task_chain_bridge.rs` — `Task.chain_id` ↔ `ChainState.task_id` bidirectional link (via `TaskEngine::link_to_chain` + `link_task_to_chain`). Pure `project_chain_status_to_task_state` maps `Running` / `WaitingSubChain` / `WaitingParallel` → `Working`; terminal chain states → matching terminal task states (`TimedOut` → `Failed`). `Gateway::advance_chain` and `cancel_chain` project automatically on completion; A2A `tasks/cancel` on a chain-backed task propagates to `Gateway::cancel_chain`. On a **terminal** projection the chain's step results fold into the task — one `Task.artifacts` entry per step (plus parallel sub-steps, the body carried as a data part) and a summary `Task.history` message — so a federated A2A caller gets the output through the standard protocol. Step bodies over the 256KB part cap are summarized in place; full bodies stay queryable through the chain API. This data projection is best-effort and never blocks the task's terminal transition. The `InputRequired` / `AuthRequired` ↔ generalized `BusApproval` half landed in #166 (PauseKind generalization).
...
- [x] **Native Push Delivery:** Implement task-scoped webhook delivery (`Create/Get/List/Delete TaskPushNotificationConfig`). Reuse shared `reqwest::Client`, retry + DLQ, and audit-stamped envelope pattern from the webhook provider. Shipped across 4.1 (config CRUD) and 4.2 (delivery worker) below; the persistent DLQ deferred in 4.2 landed afterward in #189, and the SSRF guard on delivery URLs in #192.
...
- [x] **Security review:** Audited the new endpoints. Findings:
  - **SSRF in the push delivery worker (fixed).** A `TaskPushNotificationConfig.url` is fully attacker-controlled and the worker POSTs task events to it; the only prior validation was an `http(s)`-scheme check. A malicious tenant could point a config at `169.254.169.254` (cloud metadata), `127.0.0.1` (co-located internal services), or any RFC-1918 address. Fixed with `crates/server/src/api/a2a_ssrf.rs`: a host classifier blocking loopback / private / link-local / CGNAT / unique-local / IPv4-mapped-loopback ranges plus internal hostnames. Applied at config *registration* (synchronous, literal) and at *delivery* (async, DNS-resolving — the authoritative guard against a hostname that resolves into a blocked range). A blocked delivery is terminal, lands in the DLQ, and bumps the `deliveries_ssrf_blocked` metric. Residual DNS-rebinding window (resolve→connect) documented as a hardening follow-up.
  - **`/.well-known/agent.json` (assessed — no change needed).** Unauthenticated by A2A spec; returns only operator-published `AgentCard` data, never resource-existence or tenant-internal state. The aggregation scan is now cache-fronted (#188), removing the enumeration-amplification concern.
  - **`/a2a/rpc` JSON-RPC (assessed — covered).** Codec totality is fuzz-proven (#190); body size capped at `A2A_MAX_BODY_BYTES`; errors mask `State`/`Serde` internals (CWE-209) per the Phase 2 codec work; per-tenant grant authorization enforced before dispatch.
- [x] **Load test:** `criterion` benchmark at `crates/gateway/benches/a2a_task_lifecycle.rs` covering the `TaskEngine` hot path — individual mutations (`create_task`, `transition_task`), the full lifecycle (`create → Working → append → artifact → Completed`), the same lifecycle with a stream broadcast and N ∈ {0,1,8,64} subscribers registered, the same with N ∈ {1,8,64} *actively-drained* subscriber tasks, and the stale-task reaper. Key result: the streamed lifecycle holds flat (~16µs) across all subscriber counts, confirming the emit path is O(1) in subscriber count. Run with `cargo bench -p acteon-gateway --bench a2a_task_lifecycle`.
- [x] **Fuzz the codecs:** `proptest` fuzzing on the JSON-RPC and REST codec layers — guarantees no panics on malformed input. Four property tests in `crates/server/src/api/a2a.rs` (256 cases each): a recursive arbitrary-`serde_json::Value` strategy biased toward JSON-RPC-meaningful strings drives `handle_rpc_payload` (the full dispatch path including batch arrays), the raw-string `serde_json::from_str` → dispatch entry path, the `Deserialize` totality of all six param structs (`MessageSendParams`, `TaskQueryParams`, `TaskIdParams`, `PushConfig{Set,Id,List}Params`), and a contract check that scalar payloads always yield a single error reply. `proptest` added as a dev-dependency only — no production build impact.
- [x] **Per-tenant cap overrides** — **assessed, not needed for v1.** The original bullet hedged this with "(if needed)"; the assessment is that it is not. The Phase 1 caps are generous memory-safety bounds, not throughput throttles: `Part` text/raw/data at 256 KiB each, reference-graph depth 5 / width 32, and a 30-minute default working TTL. `working_ttl_ms` is already a per-task field (settable up to `MAX_WORKING_TTL_MS` = 7 days) so the one cap a long-running agent actually bumps into is *already* caller-overridable. No reported requirement exists for raising the byte/depth caps, and doing so generically would mean threading a tenant-limits struct through every `Part`/`Message`/`Task` `validate()` plus introducing new absolute ceilings above the memory-safety bounds — speculative complexity for a hypothetical need. If a concrete high-throughput tenant requirement appears later, the cleanest home is a per-tenant `A2aTenantLimits` row resolved at the API boundary; this is recorded as a future option, not v1 scope.

### Phase 6: SDK & Simulation — ~5 days
- [x] Update all polyglot SDKs (Rust, Python, Node, Go, Java) to support the native A2A Task primitives. Each SDK ships nine methods covering the full surface (`a2a_send_message` / `a2a_get_task` / `a2a_cancel_task` / four push-config CRUD methods / `a2a_discover_agent` / `a2a_get_authenticated_extended_card`) plus per-language factory helpers for the common `Part` / `TaskMessage` / `PushNotificationConfig` shapes. Every authenticated call sends `A2A-Version: 1.0`; the discovery call is explicitly unauthenticated per spec. Wire payloads stay as language-native dict/map types — pinned record types would force a translation layer for every A2A schema change. Per-SDK test surface includes URL building, header presence, `:cancel` verb survival, percent-encoding of reserved characters in path segments, and JSON-RPC envelope round-trip. Shipped in PRs Rust (#182), Python (#183), Node (#184), Go (#185), Java (#186).
...
- [x] **README feature-matrix update** — `README.md` now carries a dedicated "Agent Interop (A2A Protocol v1.0)" section between "Pluggable Backends" and "Enterprise Ready", covering the Task Engine, dual transports, pause-for-human, artifact-stream gatekeeper, SSE events, push notifications, discovery, and multi-tenant scoping. **CHANGELOG entry skipped** — the repo does not maintain a `CHANGELOG.md` file as a convention (verified by searching for any release-notes/version-history file under the tree); creating one ad hoc for A2A would introduce a new project norm. If a CHANGELOG is later adopted, the A2A feature should land in an "Added" section spanning v0.X.
