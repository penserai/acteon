...
fn cli_replays_virtual_time_and_rejects_remote_ttls_or_forged_clocks() {
    ...
    std::fs::write(workspace.0.join("suite.json"), r#"{"schema_version":2,"seed":42,"backend":"memory","trials":2,"scenarios":["deadline_safety","worker_lifecycle","durable_scheduling","recurring_dispatch_recovery","queue_recovery","task_handoff_recovery","chain_write_fencing","chain_discovery_recovery","cancellation_handoff_recovery","chain_task_projection_recovery"]}"#).unwrap();
    let output = workspace.run(&["--manifest", "suite.json", "--output", "first"]);
    assert!(
        output.status.success(),
        "{}",
