...
#[tokio::test]
async fn prometheus_alert_rules_returns_generated_yaml() {
    let state = build_test_state(vec![]);
    let app = build_app(state);
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri("/v1/metrics/alerts/prometheus.yaml")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    assert!(yaml.starts_with("# Generated by Acteon"));
    assert!(yaml.contains("ActeonMetricsUnavailable"));
}
...
async fn openapi_json_is_valid() {
    ...
    assert!(
        paths.contains_key("/v1/metrics/alerts/prometheus.yaml"),
