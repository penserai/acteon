...
pub struct AlertmanagerImportArgs {
    ...
    /// Default namespace stamped onto generated time intervals (and any
    /// future per-tenant resources). Rules and providers are global and
    /// not affected.
    #[arg(long, default_value = "default")]
    pub default_namespace: String,

    /// Default tenant stamped onto generated time intervals. Override
    /// when importing for a specific tenant; the value is written
    /// verbatim into `time-intervals.yaml`.
    #[arg(long, default_value = "default")]
...
"##;
    ...
    fn convert_time_intervals_renders_yaml() {
        ...
        let yaml = render_time_intervals_yaml(&intervals, "prod", "acme");
    ...
    #[test]
    fn render_time_intervals_default_tenant_substituted() {
        // Independent check that the tenant string is rendered verbatim
        // (no hidden lowercasing or escaping).
        let config: AlertmanagerConfig =
            serde_yaml_ng::from_str(TIME_INTERVAL_CONFIG).expect("parse");
        let intervals = convert_time_intervals(config.time_intervals.as_ref().unwrap());
        let yaml = render_time_intervals_yaml(&intervals, "default", "team-payments");
        assert!(yaml.contains("tenant: team-payments"));
