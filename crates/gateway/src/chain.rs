...
pub fn resolve_template(
    ...
    parallel_sub_results: &HashMap<String, StepResult>,
) -> serde_json::Value {
    match template {
...
///
/// Searches the main `step_results` first, then falls back to
/// `parallel_sub_results` for results produced by parallel sub-steps.
fn resolve_named_step(
    ...
    // Find in main step results first.
    let result = steps_config
        .iter()
        .position(|s| s.name == name)
        .and_then(|idx| step_results.get(idx))
        .and_then(|r| r.as_ref())
        // Fall back to parallel sub-results.
        .or_else(|| parallel_sub_results.get(name));
...
mod tests {
    ...
    // -- parallel sub-results template resolution tests ---------------------------

    #[test]
    fn resolve_parallel_sub_result_by_name() {
        let origin = test_origin();
        let steps = test_steps_config();
        let mut parallel_sub_results = HashMap::new();
        parallel_sub_results.insert(
            "notify_slack".to_owned(),
            StepResult {
                step_name: "notify_slack".to_owned(),
                success: true,
    ...
    #[test]
    fn resolve_parallel_sub_result_main_steps_take_priority() {
        // If a name exists in both main steps and parallel sub-results,
        // main steps should win.
        let origin = test_origin();
        let steps = test_steps_config();
        let results = vec![step_result("search", serde_json::json!({"source": "main"}))];
        let mut parallel_sub_results = HashMap::new();
        parallel_sub_results.insert(
            "search".to_owned(),
            StepResult {
                step_name: "search".to_owned(),
                success: true,
        ...
        // Main step result should take priority.
