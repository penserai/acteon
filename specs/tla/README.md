...
| **Recurring Dispatch** | `RecurringDispatch.tla` | `background/workers/recurring.rs` claim lease + index re-arm (the PR #235/#304 fixes) | Each occurrence is dispatched **at most once** when dispatch outlives its lease; the model assumes Rust's two-poll-plus-margin lease contract, exercised by `scenarios/recurring-dispatch.json` |
