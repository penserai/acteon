...
function mockTemplatesApi(
  ...
  return page.route('**/v1/**', async (route) => {
    ...
    // Render preview (must be checked before template-by-ID to avoid "render" matching as an ID)
    if (url.includes('/v1/templates/render') && route.request().method() === 'POST') {
      const reqBody = route.request().postDataJSON()
      const profileName = reqBody?.profile ?? ''
      // Simulate rendered output based on the profile name
      const rendered: Record<string, string> = {}
      if (profileName === 'email-alert') {
        rendered['subject'] = 'Alert: CRITICAL - payment-service is degraded'
        rendered['body'] = `<!DOCTYPE html>\n<html>\n<body style="font-family: sans-serif; background: #f9fafb; padding: 24px;">\n  <div style="max-width: 600px; margin: 0 auto; background: #fff; border-radius: 8px; padding: 32px;">\n    <h1 style="color: #dc2626;">CRITICAL: payment-service</h1>\n    <p>Response latency exceeds 2000ms threshold for the last 5 minutes.</p>\n    <table style="width: 100%; border-collapse: collapse; margin-top: 16px;">\n      <tr>\n        <td style="padding: 8px; border: 1px solid #e5e7eb; font-weight: 600;">region</td>\n        <td style="padding: 8px; border: 1px solid #e5e7eb;">us-east-1</td>\n      </tr>\n      <tr>\n        <td style="padding: 8px; border: 1px solid #e5e7eb; font-weight: 600;">pod</td>\n        <td style="padding: 8px; border: 1px solid #e5e7eb;">payment-svc-7f8b9c-xk2lm</td>\n      </tr>\n    </table>\n    <p style="color: #6b7280; font-size: 12px; margin-top: 24px;">Generated at 2026-02-19T14:30:00Z</p>\n  </div>\n</body>\n</html>`
        rendered['footer'] = 'This alert was generated by Acteon &bull; <a href="https://acteon.example.com/silence/abc123">Silence for 1h</a>'
      } else if (profileName === 'order-receipt') {
        rendered['subject'] = 'Your order #ORD-2026-0042 is confirmed \u2014 estimated delivery March 3, 2026'
        rendered['html_body'] = `<!DOCTYPE html>\n<html>\n<body style="font-family: sans-serif;">\n  <h1>Thank you for your order, Alice!</h1>\n  <p>Order <strong>#ORD-2026-0042</strong> has been confirmed.</p>\n  <table>\n    <tr><td>Widget Pro</td><td>2</td><td>$49.99</td></tr>\n    <tr><td>USB-C Cable</td><td>1</td><td>$12.99</td></tr>\n  </table>\n  <p><strong>Total: $112.97</strong></p>\n</body>\n</html>`
        rendered['reply_to'] = 'orders@acme.com'
      } else {
        rendered['output'] = `Rendered output for profile "${profileName}"`
      }
      await route.fulfill({
        status: 200,
    ...
    // Individual template by ID (must be after /profiles and /render checks)
...
test.describe('Template Screenshots', () => {
  ...
  test('templates-playground — Playground tab with rendered output', async ({ page }) => {
    await mockTemplatesApi(page)
    await page.goto(`${BASE_URL}/templates`, { waitUntil: 'networkidle' })
    await page.waitForTimeout(500)
    ...
    // Select the email-alert profile from the dropdown
    await page.locator('#playground-profile').selectOption('email-alert')
    await page.waitForTimeout(200)

    // Fill in realistic test payload
    await page.locator('#playground-payload').fill(JSON.stringify({
      severity: 'critical',
      service: 'payment-service',
      status: 'degraded',
      message: 'Response latency exceeds 2000ms threshold for the last 5 minutes.',
      timestamp: '2026-02-19T14:30:00Z',
      silence_url: 'https://acteon.example.com/silence/abc123',
      details: {
        region: 'us-east-1',
        pod: 'payment-svc-7f8b9c-xk2lm',
      },
    }, null, 2))
    await page.waitForTimeout(200)
    ...
    await page.screenshot({
      path: path.join(ASSETS, 'templates-playground.png'),
      fullPage: true,
    })
  })
})
